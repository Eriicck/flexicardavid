...
export default function Login() {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 font-sans text-slate-800">
      ...
      {/* TARJETA PRINCIPAL (Más compacta y centrada) */}
      <div className="relative z-10 w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row shadow-black/50">
        
        {/* LADO IZQUIERDO (Imagen y Branding - Oculto en móvil muy pequeño, visible en tablet/pc) */}
        <div className="hidden md:flex md:w-5/12 bg-slate-900 relative flex-col justify-between p-8 text-white">
           ...
           <form onSubmit={handleLogin} className="space-y-4">
              <div className="space-y-3">
                  <div className="relative group">
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 pl-10 pr-4 text-sm font-medium outline-none focus:border-orange-500 focus:bg-white transition-all"
              ...
              <div className="pt-2">
                 ...
                 <div className="grid grid-cols-1 gap-2">
                    ...
                    <div onClick={() => setSelectedRole('comercial')} className={`flex items-center gap-3 p-2.5 rounded-lg border cursor-pointer transition-all ${selectedRole === 'comercial' ? 'border-orange-500 bg-orange-50 ring-1 ring-orange-500' : 'border-slate-100 hover:border-slate-300'}`}>
                    ...
                    <div onClick={() => setSelectedRole('admin')} className={`flex items-center gap-3 p-2.5 rounded-lg border cursor-pointer transition-all ${selectedRole === 'admin' ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500' : 'border-slate-100 hover:border-slate-300'}`}>
                    ...
                    <div onClick={() => setSelectedRole('cliente')} className={`flex items-center gap-3 p-2.5 rounded-lg border cursor-pointer transition-all ${selectedRole === 'cliente' ? 'border-green-500 bg-green-50 ring-1 ring-green-500' : 'border-slate-100 hover:border-slate-300'}`}>
                         <div className={`p-1.5 rounded-md ${selectedRole === 'cliente' ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-500'}`}><Users size={16}/></div>
                        <div className="flex-1"><p className={`text-xs font-bold ${selectedRole === 'cliente' ? 'text-green-900' : 'text-slate-700'}`}>Cliente / Web</p></div>
                        {selectedRole === 'cliente' && <CheckCircle size={14} className="text-green-600"/>}
                    </div>
